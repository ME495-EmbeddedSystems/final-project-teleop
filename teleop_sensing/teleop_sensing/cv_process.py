...
class ImageProcesser(RosNode):
    ...
    def __init__(self) -> None:
        ...
        # The non alighted depth image is not usable. points near the corner of the image
        # will have huge bad offset.
        self.depth_image_sub = self.create_subscription(
            RosImage,
            "/D435i/aligned_depth_to_color/image_raw",
            functools.partial(self._push_to_queue_callback, self.depth_image_deque),
            10,
        )
        self.depth_ci_sub = self.create_subscription(
            CameraInfo,
            "/D435i/aligned_depth_to_color/camera_info",
            functools.partial(self._push_to_queue_callback, self.depth_cam_info_deque),
            10,
        )
        ...
        self.raw_image_sub = self.create_subscription(
            ...
            functools.partial(self._push_to_queue_callback, self.raw_image_deque),
            10,
        )
        self.raw_ci_sub = self.create_subscription(
            CameraInfo,
            "/D435i/color/camera_info",
            functools.partial(self._push_to_queue_callback, self.raw_cam_info_deque),
            10,
        )

        # intentionally to be between 1~2 times the image publishing rate.
        # So the timer is likely in between when all image topics
        # has done publishing.
        self.process_timer = self.create_timer(1 / 50, self._find_ring_process_time_cb)
        print("Done Initializing")

    def _push_to_queue_callback(self, target_queue: collections.deque, msg):
        """
        Push the received msg to the given queue.
    ...
    def _find_ring_process_time_cb(self):
        ...
        for name, time in named_time.items():
            ...
            if d_time > (1 / 20):
                print(f"{name} is {d_time} behind {base_name}! Above " +
                      "the 1 / 20(fps) limit Skipping cycle due to this")
                return
        """
        ...
        # Depth camera info is not used for now.
        _ = self.depth_cam_info_deque.pop()
        ...
        for c, h_filter in self.color_filter_map.items():
            ...
            raw_blob_marked, raw_pxy = self._find_donute_with_color_mask(raw_img_cv, full_hsv_mask)
            if self.debug_mode:
            ...
            print(f"Color {c.name} at xyz {raw_rect_ph_sxyz[0]:.5f} " +
                  f"{raw_rect_ph_sxyz[1]:.5f} {raw_rect_ph_sxyz[2]:.5f} ")

            self._publish_circle_tf(depth_img.header, f"{c.name.lower()}_center", raw_rect_ph_sxyz)

    def _find_donute_with_color_mask(self, bgr_image, color_mask):
        """
        Find the ring given the image and the color mask.
        ...
        pxy = None
        if keypoints:
            pxy = self._keypoint_to_pxy(self.get_largest_keypoint(keypoints))

        return blob_marked, pxy

    def get_largest_keypoint(self, keypoints: list):
        """
        Get the largest keypoint.
    ...
    def _publish_circle_tf(self, image_header: Header, child_frame_id: str, sxyz: list[float]):
        ...
        Args:
        ----
            image_header (Header): the header of the image, will be the header for the tf
            child_frame_id (str): frame name
            sxyz (list[float]): the xyz position of ring in space
    ...
    def _get_transform(self, target_frame, source_frame) -> Optional[TransformStamped]:
        """
