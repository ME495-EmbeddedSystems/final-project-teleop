...
Takes in joint states and publishes them to a ros_controller that
then controls the left hand in gazebo
...
def createJointNamedict():
    ...
    -------
        jointDict (dictionary): The dictionary that maps
            joint names from the haptx to joint names on the
            left hand model
...
class Ros_gz_joint_client(Node):
    ...
    def __init__(self):
        ...
        self.haptx_sub = self.create_subscription(
            JointState, "/left_hand/joint_states", self.jointStateListener, 10
        )

        # Action client
        self.ControllerAvailable = False
        self.callbackGroup = MutuallyExclusiveCallbackGroup()
        self.Mover = ActionClient(
            self,
        ...
        self.message.goal_time_tolerance = Duration(
            seconds=1.0
        ).to_msg()  # Start imediately

        self.message.trajectory.joint_names = [
            "palm_x",
            "palm_y",
            "palm_z",
            "palm_roll",
    ...
    def jointStateListener(self, msg=JointState):
        ...
        Args:
        ----
            msg (JointState): Joint states published to
            /left_hand/joint_states. Defaults to JointState.
        ...
        for i in range(len(msg.name)):
            ...
            # positions in point2
            self.point2.positions[self.jointDict[msg.name[i]]] = \
                msg.position[i]
